...
if (SessionManager.getUserId(sessionId) != null) {
	...
	function addEmail() {
		if (emailCount < 5) {
			...
			const emailContainer = document
					.getElementById('contactEmailContainer');
	...
	function addPhone() {
		if (phoneCount < 5) {
			...
			const phoneContainer = document
					.getElementById('contactPhoneContainer');
			const newDiv = document.createElement('div');
	...
	<div id="profile">
		...
		<table>
			<tr>
				...
				<td><a href="editemails.jsp"><input type="button"
						id="editPrimaryEmail" value="Set Primary Email"></a></td>
			</tr>
		</table>
		...
		<form name="myEmailForm" action="DeleteEmailServlet" method="post">
			...
			%>
			<input type="submit" name="<%=email.getUserEmail()%>" value="❌"
				onclick="deleteClickedEmail(this, event)"> <br>
			<br>
			<%
}
%>
...
%>
	...
	<form action="AddContactServlet" method="post">
		<label>Name:</label> <input type="text" name="contName"
			placeholder="Enter your Name" required><br> <label>Date
			of Birth:</label> <input type="date" name="contDateOfBirth"
			placeholder="Enter your Date of Birth" required><br> <label>Age:</label>
		<input type="text" name="contAge" placeholder="Enter your Age"
			required><br> <label>State:</label> <input type="text"
			name="contState" placeholder="Enter your State" required><br>
		<label>City:</label> <input type="text" name="contCity"
			placeholder="Enter your City" required><br> <label>Phone
			Number:</label>
		<div id="contactPhoneContainer">
			<input type="text" name="contactPhone"
				placeholder="Enter Phone Number 1" id="phone1" required>
			<button type="button" onclick="addPhone()">+</button>
			<button type="button" onclick="removePhone()">-</button>
		</div>
		<br> <label>Email Id:</label>
		<div id="contactEmailContainer">
			<input type="email" name="contactEmail"
				placeholder="Enter Email Address 1" id="email1" required>
			<button type="button" onclick="addEmail()">+</button>
			<button type="button" onclick="removeEmail()">-</button>
	...
	<form>
		<input type="submit" value="View Contacts" formaction="contacts.jsp"
			formmethod="post">
	</form>
	<br>
	<hr>
 ...
 -->
	...
	<form action="CreateGroupServlet" method="post">
		<input type="text" name="categoryName"
			placeholder="Enter Category Name"> <input type="submit"
	...
	%>
	<a
		href="viewcategory.jsp?categoryId=<%=category.getCategoryId()%>&categoryName=<%=category.getCategoryName()%>">View
		<%=category.getCategoryName()%></a> -
	<a
		href="DeleteCategoryServlet?categoryId=<%=category.getCategoryId()%>">Delete
		<%=category.getCategoryName()%></a>
	<br>
	<%
	}
